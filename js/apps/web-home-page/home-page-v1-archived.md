# Cura — Marketing Home Page: V1 (Archived)

## Project Status: ✅ Complete (all 63 tasks shipped)

> **Archived 2026-03-20.** This document covers the original home page build (HP-001 to HP-063). All tasks are complete and shipped. For the active rework plan (HP-064+), see `home-page-project-management.md`.
...
### Gaps they do NOT address (Cura's exclusive territory — V1)
...
### EPIC-HP-004B: Replace Your Stack Section
...
- [x] **HP-023:** Four-column pain cards, each with an icon, bold label, and one-sentence description:
  - **"The Endless Click"** — `"11 clicks to move a candidate in most ATS tools. We counted."` (lightning)
  - **"Context Amnesia"** — `"You switch between email, LinkedIn, and CRM and lose the thread every single time."` (brain)
  - **"Slow = Lost"** — `"The recruiter who replies in minutes places the candidate. You're still searching the inbox."` (timer)
  - **"The Tool Tax"** — `"ATS. Outreach tool. Enrichment API. LinkedIn Recruiter. You're paying for four tools and managing five context switches — daily."` (money) — directly counters Loxo's "Headache Tax" framing with our own sharper version
...
- [x] **HP-061:** Create a visual "old stack vs Cura" comparison — left side shows 5 greyed-out tool-pill logos (generic: `ATS`, `LinkedIn Recruiter`, `Email Sequencer`, `Enrichment API`, `Calendar Sync`) with a red "x" strike; right side shows a single Cura logo with a glowing indigo halo; an animated arrow sweeps left to right
...
- [x] **HP-033:** Create tabbed showcase with three tabs (`Match`, `Draft`, `Brief`) — clicking each swaps the displayed mock UI panel:
...
- [x] **HP-036:** Build `<KeyboardBadge>` component — small dark card showing keyboard shortcut examples: `Cmd+K` (command palette), `G C` (go to candidates), `A` (approve draft) — styled like a terminal / OS key-cap UI
...
- [x] **HP-039:** Render comparison table. Use archetypes (no direct competitor names) — `"Legacy Enterprise"` (Bullhorn), `"Talent Intelligence"` (Loxo), `"Scale & Volume"` (RecruitCRM), `"Cura"`. Rows derived from actual feature gaps identified in Q1 2026 research:
  ...
  | ----------------------------------------- | :---------------: | :-----------------: | :------------: | :------: |
  | Autonomous pipeline movement              |         x         |       partial       |       x        |   yes    |
  | Zero-loss engagement memory               |         x         |          x          |       x        |   yes    |
  | Unified context (Zoom + email + LinkedIn) |         x         |          x          |       x        |   yes    |
  | Human-in-the-loop approval gates          |         x         |          x          |       x        |   yes    |
  | Natural language search                   |         x         |     paid add-on     |       x        |   yes    |
  | Keyboard-first speed UX                   |         x         |          x          |       x        |   yes    |
  | EU AI Act compliance-ready                |         x         |          x          |       x        |   yes    |
  | Boutique-friendly pricing                 |         x         |          x          |       x        |   yes    |
  | Full lifecycle: source to offer           |      partial      |       partial       |      yes       |   yes    |
...
- [x] **HP-058:** Add `"(c) 2026 Cura. Built for the humans who build careers."` copyright line — small, muted
- [x] **HP-059:** Add GDPR cookie-consent banner component (minimal, bottom-bar, dark-styled); consent state stored in `localStorage`

---

## V1 Section Map (Scroll Order)
