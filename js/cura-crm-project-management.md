...
- [x] **WA-057:** Add delete button with confirmation dialog (warn about linked jobs)
- [x] **WA-058:** Wire to `deleteClient` mutation with cascade handling
...
- [x] **TASK-038:** Create Job GraphQL schema with Client relationship
- [x] **TASK-039:** Implement Job service layer
- [x] **TASK-043:** Add Job deletion and archiving

#### Story 5.2: Job API — Status Management (BE)

- [x] **TASK-044:** Implement job status workflow (open, on-hold, filled, closed)
- [x] **TASK-045:** Add job priority levels
- [x] **TASK-046:** Create recruiter assignment functionality
- [x] **TASK-047:** Add job requirements and description rich text editor

#### Story 5.3: Job List Page (FE)

> Paginated table of jobs with client relationship and status.

- [x] **WA-059:** Create `/jobs` route with `PageHeader` ("Jobs" + "Add Job" button)
- [x] **WA-060:** Build `JobTable` — columns: title, client name, status, priority, assigned recruiter, created
- [x] **WA-061:** Connect to `jobs` GraphQL query with loading skeleton and empty state
- [x] **WA-062:** Add pagination, search (title/client), status filter, priority filter, and sort controls
- [x] **WA-063:** Add row click → navigate to job detail page

#### Story 5.4: Job Create & Edit (FE)

> Form to create or edit a job with client selector.

- [x] **WA-064:** Create `/jobs/new` route with job form
- [x] **WA-065:** Build `JobForm` component — fields: title, client (searchable selector), description (rich text or textarea), status, priority, assigned recruiter
- [x] **WA-066:** Build `ClientSelector` — searchable dropdown that queries clients, shows selected client info
- [x] **WA-067:** Add form validation and wire to `createJob` mutation
- [x] **WA-068:** Create `/jobs/[id]` detail route with job info + linked candidates section (placeholder for Phase 2 pipeline)
- [x] **WA-069:** Add inline edit mode on detail page, wired to `updateJob` mutation

#### Story 5.5: Job Delete & Status (FE)

- [x] **WA-070:** Add delete button with confirmation dialog
- [x] **WA-071:** Add status change controls on detail page (Open → On Hold → Filled → Closed) with one-click transitions
...
- **EPICs:** 0/6 Complete
- **Stories:** 24/34 Complete
- **Tasks:** 120/165 Complete (46 BE + 74 FE)
...
**Active Stories:** Story 5.5 complete. Sprint 6 fully delivered (Stories 4.5, 5.1, 5.2, 5.3, 5.4, 5.5).
**Blocked Items:** None
**Next:** Sprint 7 — Dashboard (Story 6.1) and Command Palette (Story 6.2)
...
| 2026-05-18 | Story 4.5 complete — Client Delete (FE) | Delete button + `AlertDialog` added to `/clients/[id]` page header next to Edit (red-tinted outline button, `Trash2` icon, mirrors candidate delete pattern). Dialog title `Delete {client.name}?`. Dialog description is conditional on `client.totalJobCount`: `0` → "This cannot be undone."; `>0` → "This will also archive N linked job(s) (M currently active). This cannot be undone." Pluralisation handled inline. `deleteClient` mutation wired with `refetchQueries: [{ query: ClientsDocument, variables: { first: 20 } }]`, `awaitRefetchQueries: false` so the list is fresh on return. Success toast appends an "N linked jobs archived." note when applicable, then `router.push("/clients")`. Apollo errorLink already surfaces permission failures via toast. Backend cascade already in place (`ClientService.softDelete` does `job.updateMany({ where: { clientId, deletedAt: null }, data: { deletedAt: now } })` at `client.service.ts:244-247`) — warning copy matches reality. Verified end-to-end via Playwright on the seeded "test" (0-jobs) client: detail page shows Edit + Delete in header; clicking Delete opens dialog with title "Delete test?" and description "This cannot be undone."; Cancel dismisses cleanly with no toast. `tsc --noEmit` passes (only pre-existing `.next/types/validator.ts` route errors remain). WA-057, WA-058 complete |
| 2026-05-18 | Story 5.3 complete — Job List Page (FE) | **Schema sync + new GraphQL docs:** copied API's authoritative `schema.gql` → `web-app/schema.graphql` (picks up `JobConnection`/`JobEdge`/`JobFilterInput`/`JobSortField`/`requirements` and the new `updateJobStatus`/`updateJobPriority`/`assignJob` mutations). New `src/graphql/jobs/` folder: `job-fragments.graphql` (JobFields fragment incl. `requirements`), `job-queries.graphql` (`Jobs` with full Relay args + nested `client { id name }` + `assignedTo { id firstName lastName email }`, `Job` single-record query with `ownerUser` too), `job-mutations.graphql` (`CreateJob`/`UpdateJob`/`DeleteJob`/`UpdateJobStatus`/`UpdateJobPriority`/`AssignJob`); `pnpm codegen` regenerated typed hooks. **Backend ResolveFields:** added `@ResolveField assignedTo` and `@ResolveField ownerUser` to `JobResolver` via `loaders.userById` (DataLoader-batched); both return `UserModel | null`. Wired `DataLoaderService` into the resolver constructor. **Permission bugfix:** `PermissionService.effectiveLevel` now falls back to `getDataScope(user, resourceType)` after explicit grants + cascade — if the user's data scope is `ALL`, returns implicit `VIEW`. Without this, admin roles could list rows (data-scope filter passes) but couldn't traverse to them via ResolveField (which calls `assertCan(VIEW)`), so the Jobs page erroring on `Access denied: VIEW on Client/…` while resolving the `client { id name }` nested field. **UI components:** `JobStatusBadge` (Open=emerald, ON_HOLD=amber, Filled=blue, Closed=zinc); `JobPriorityBadge` (Low=zinc, Medium=sky, High=orange, Urgent=red); `JobTable` with columns Title/Client/Status/Priority/Recruiter/Created (recruiter falls back to italic "Unassigned"; client name via `j.client?.name`; createdAt via shared relative-time formatter), skeleton + "No jobs yet" empty state, row click → `/jobs/<id>`; `JobFilters` mirroring `ClientFilters` (debounced 300ms search via `useDebouncedValue`, Status dropdown, Priority dropdown, Sort field dropdown with TITLE/CREATED_AT/UPDATED_AT/PRIORITY, ASC/DESC arrow toggle, Clear pill, mount-gate to avoid Radix Select+Suspense hydration mismatch); `JobPagination` (prev/next + "Showing N of M • 20 per page"). **/jobs page:** `<Suspense>`-wrapped inner component reads initial state from URL (`q`/`status`/`priority`/`sort`/`order`), pushes to URL via `router.replace`, resets cursor on every filter change, skips query until `useAuth().user` hydrates, surfaces errors in destructive banner. **Verified end-to-end via Playwright on seeded data:** `/jobs` shows all 4 jobs (Senior Data Engineer/FILLED, VP of Engineering/OPEN+Urgent, Staff Frontend Engineer/OPEN+Medium, Senior Backend Engineer/OPEN+High) with color-coded badges and recruiter names (Bob Recruiter / Alice12 Admin). `/jobs?status=OPEN&sort=TITLE&order=ASC` restores the Open status filter, Title sort, ascending arrow, Clear pill, and re-sorts to 3 open jobs alphabetically. `tsc --noEmit` clean (only pre-existing `.next/types/validator.ts` Next.js codegen warnings). WA-059–WA-063 complete. |
| 2026-05-18 | Story 5.2 complete — Job API Status Management (BE) | **TASK-047 — `requirements` field:** Added `requirements String?` to `Job` model in `prisma/schema.prisma` + migration `20260518000000_add_job_requirements` (`ALTER TABLE "jobs" ADD COLUMN "requirements" TEXT`); applied via `prisma migrate deploy`; regenerated Prisma client. Exposed on `JobModel`, `CreateJobInput`, `UpdateJobInput` (via `PartialType`) with `@Transform` trim + `@IsString`. Persisted in `JobService.create` and `JobService.update`. **TASK-044 — Status workflow:** New `ALLOWED_TRANSITIONS` map enforces OPEN→{ON_HOLD,FILLED,CLOSED}, ON_HOLD→{OPEN,FILLED,CLOSED}, FILLED→{OPEN,CLOSED}, CLOSED→{OPEN} (reopen paths kept so recruiters can recover from fell-through placements). New `JobService.changeStatus(id, user, status)` validates the transition (throws `BadRequestException` on invalid moves), auto-sets `closeDate=now()` when entering FILLED/CLOSED (only if not already set), clears `closeDate=null` when reopening, and adjusts `client.activeJobCount` on OPEN↔non-OPEN flips. New `updateJobStatus(id, status)` mutation. **TASK-045 — Priority:** New `JobService.changePriority(id, user, priority)` + `updateJobPriority(id, priority)` mutation for one-click priority toggles. **TASK-046 — Recruiter assignment:** New `JobService.assign(id, user, assignedToId | null)` validates the assignee exists in tenant + is `loginable` (`BadRequestException` otherwise), sets `assignedToId`, and auto-grants `EDIT` permission to the new assignee via `permissionService.grant` (`grantSource: RULE`). Passing `null` unassigns and skips the grant. New `assignJob(id, assignedToId)` mutation. All three new mutations call `assertCan(user, "Job", id, EDIT)` after `findById` (which itself checks `VIEW`). Verified end-to-end via live GraphQL introspection — `updateJobStatus`, `updateJobPriority`, `assignJob` mutations present with correct arg shapes; `JobModel.requirements` exposed as `String`. `tsc --noEmit` passes. TASK-044, TASK-045, TASK-046, TASK-047 complete. |
| 2026-05-18 | Story 5.1 complete — Job API CRUD (BE) | **JobService rewritten** to mirror Client/Candidate patterns: `RequestUser`-driven, data-scope filtering (`getDataScope("Job")` → ALL/TEAM_TREE/MY_TEAMS/MINE/EXPLICIT), row-level `assertCan(user,"Job",id,VIEW)` on `findById`, `EDIT` on `update`/`softDelete`. `create()` now validates `clientId` belongs to the tenant (`BadRequestException` if missing/deleted), defaults `ownerUserId` to creator, owns client rollups directly (`totalJobCount` always incremented, `activeJobCount` only when status=OPEN). `update()` validates new `clientId` when reassigning and flips `activeJobCount` on OPEN↔non-OPEN transitions. `softDelete()` decrements `activeJobCount` when archiving an open job. Dead `ClientService.adjustActiveJobCount` helper removed (only the old JobService called it). **Relay pagination + filters:** new `JobConnection`/`JobEdge` (`src/common/graphql/models/job-connection.model.ts`) registered in models index; new `JobFilterInput` (search across title/description, status, priority, clientId, assignedToId — all trimmed/case-insensitive); new `JobSortField` enum (TITLE/CREATED_AT/UPDATED_AT/PRIORITY, default UPDATED_AT DESC, reuses `SortOrder` from candidate-sort); cursor pagination via base64url(id), `first`+`after`/`last`+`before` with combo rejection (`BadRequestException`), page size default 20 max 100, secondary `id` ordering for stable paging. **JobResolver upgraded:** `jobs` query now returns `JobConnection` with `first/after/last/before/filter/sortBy/sortOrder` args; `createJob` gains `@RequirePermission("job:create")`, `deleteJob` gains `@RequirePermission("job:delete")`; resolver still exposes `@ResolveField client` for the relationship. **CreateJobInput hardened:** `@Transform` trims on all string fields (drops blank → undefined), `@IsNotEmpty` on `clientId` and `title`, `@IsEnum` on status/priority; `UpdateJobInput` remains `PartialType(CreateJobInput)`. Verified end-to-end via live GraphQL introspection on http://localhost:8000/graphql — `JobConnection`, `JobFilterInput`, `JobSortField` and the new `jobs` args all present. `tsc --noEmit` passes. TASK-038, TASK-039, TASK-043 complete. |
...
| 2026-05-18 | Stories 5.4 + 5.5 complete — Job Create & Edit, Delete & Status (FE) | **New GraphQL doc:** `Users` query added to `user-queries.graphql` (id/firstName/lastName/email/loginable); `pnpm codegen` regenerated `UsersDocument`. **New components:** `ClientSelector` (`src/components/jobs/client-selector.tsx`) — custom combobox built on existing primitives (Button trigger + click-outside `useEffect` + portal-less dropdown with debounced search Input, no Radix Popover dependency), queries `ClientsDocument` with `skip: !open \|\| !user` and `fetchPolicy: "cache-and-network"` so opening the picker after auth hydrates fires a fresh fetch. Renders 5 most-recent clients, "No clients match…" empty state, Check icon on selection. `JobForm` (`src/components/jobs/job-form.tsx`) — single reusable form for create + edit with title (required), ClientSelector (required), 2-col Status/Priority Selects, Assigned recruiter Select (sentinel `__unassigned__` value mapped to null on submit; populated from `UsersDocument` filtered to `loginable: true`), Description textarea (6 rows), Requirements textarea (6 rows). Client validation: title non-empty, client picked. `toCreateJobInput()` trims + drops empties + asserts client selected; `toUpdateJobInput()` passes `null` for cleared optional fields (BE `JobService.update` treats undefined as untouched, null as cleared). **New pages:** `/jobs/new` (`src/app/(app)/jobs/new/page.tsx`) — back link + PageHeader + `JobForm`, `useMutation(CreateJobDocument)` with `refetchQueries: [Jobs]`, success toast `"Created <title>"`, then `router.push("/jobs/<id>")`. `/jobs/[id]` (`src/app/(app)/jobs/[id]/page.tsx`) — Next.js 15 async `params` via `use(params)`, `useQuery(JobDocument)`. View mode: status + priority badges, clickable client link → `/clients/<id>`, 2-col Recruiter/Owner (italic "Unassigned" when null), Description + Requirements sections (whitespace-pre-wrap), **Change status** section with one-click transition buttons mirroring API `ALLOWED_TRANSITIONS` (OPEN → On hold/Mark filled/Close; ON_HOLD → Reopen/Mark filled/Close; FILLED → Reopen/Close; CLOSED → Reopen) wired to `UpdateJobStatusDocument`, **Candidates** placeholder for Phase 2 pipeline. Edit toggle re-uses `JobForm` pre-filled from cache; `useMutation(UpdateJobDocument)`. Delete button + `AlertDialog` (red-tinted outline, mirrors candidate/client pattern), title `Delete {job.title}?`, body "This cannot be undone.", wired to `DeleteJobDocument` with `refetchQueries: [Jobs]` + redirect to `/jobs`. **Auth-gating fix:** both Users (JobForm) and Clients (ClientSelector) queries gated on `!!user` from `useAuth()` to avoid firing before AuthProvider's mount-effect `/api/auth/refresh` hydrates the access token — without it, the first request after a hard navigation went out without the `Authorization` header and the API rejected it with "Missing or invalid authorization header." Matches existing pattern in `JobsPage`. **Verified end-to-end with Playwright** (`/tmp/cura-pw-5.4-5.5/verify.js`): empty submit → both required errors; ClientSelector loads 5 clients, picks one; Recruiter dropdown loads 3 options (Unassigned + 2 users); create → redirects to `/jobs/<ULID>`; detail page shows correct title/badges/recruiter/owner/description/requirements + 3 transition buttons; "Put on hold" click → badge updates to "On hold"; Edit → prefilled form → title change → Save → heading updates; Delete dialog Cancel keeps dialog mounted; confirm → redirects to `/jobs` with deleted job absent from list. `tsc --noEmit` clean (only pre-existing `.next/types/validator.ts` Next.js codegen warnings). WA-064–WA-071 complete. |
