...
| `docs/talon-technical-plan.md` | Technical Design | Talon browser task executor — deterministic browser automation design: permission model (Orchestrator/Approver/Runner separation), human approval gate, site isolation (one site per session), credential fetch-and-discard flow, browser-use integration, no-task-no-action guarantee, data model (TalonTask, TalonCredential, TalonAuditLog) |
